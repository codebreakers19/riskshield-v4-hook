...
- Added hook-level tests for dynamic premium fee overrides, senior position opening through `afterAddLiquidity`, premium accounting, and `onlyPoolManager` callback protection.
- Added a deployment helper for Unichain Sepolia mock-token deployment.

## Current status

The contracts compile with `solc` using the local Uniswap `v4-core` checkout. Foundry is installed and the first test suites pass with `forge test -vv` and `forge test --gas-report`.
...
- Expand the Foundry test suite with more adverse price-path scenarios.
